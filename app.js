...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <h3 <span class="fa fa-star fs-5 text-warning fw-bold ">Rating: ${product.rating.rate}</span></h3>
      <br>
      <h3 <span class="fa fa-user fs-5 text-secondary fw-bold ">Rated By: ${product.rating.count}</span></h3>
      
      <br> <br>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-warning"><span class="text-white fw-bold fs-5">Add To Cart</span></button>
      <button id="details-btn" class="btn btn-danger fw-bold fs-5">Details</button></div>
      `;
